...
sample_metadata
```
## subset for 45 min timepoint
```{r}
# Subset metadata for time point 45
sample_metadata_45 <- sample_metadata[sample_metadata$time_point == "45", ]

# Check the subsetted metadata
print(sample_metadata_45)

# Subset count matrix for time point 45
count_matrix_45 <- filtered_count_matrix[, colnames(count_matrix) %in% rownames(sample_metadata_45)]

# Check dimensions of the subsetted count matrix
cat("Subsetted count matrix dimensions:", dim(count_matrix_45), "\n")
...
dds <- DESeqDataSetFromMatrix(countData = count_matrix_45,
                              colData = sample_metadata_45,
                              design = ~ condition)
...
top50_genes <- res_filtered_final_notrna %>%
...
write.table(top50_genes, file = "top50_genes.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
```
```{r}
# Select the top 10 downregulated genes (lowest log2 fold change)
bot50_genes <- res_filtered_final_notrna %>%
...
write.table(bot50_genes, file = "bot50_genes.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
...
## Save file as csv
```{r}
write_csv(res_filtered_final, "yeast_results.csv")
```
