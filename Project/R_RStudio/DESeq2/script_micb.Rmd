...
count_files_path <- "count_matrix"

# List the ReadsPerGene files in the specified folder
...
read_counts <- function(file) {
  counts <- read.table(file, header = FALSE, skip = 4)  # Skip header lines
  counts <- counts[, c(1, 2)]  # Select Gene ID and unstranded counts
...
```{r}
# Define sample metadata based on sample names
sample_metadata <- data.frame(
    sample = colnames(count_matrix),
    condition = c("acetic", "acetic", "acetic", "acetic", "control", "control", "control", "control"),
    time_point = c("45", "45", "120", "120", "45", "45", "120", "120"),
...
res <- results(dds, name = "condition_control_vs_acetic") %>% as.data.frame()
