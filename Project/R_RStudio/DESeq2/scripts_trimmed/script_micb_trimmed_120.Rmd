...
sample_metadata
```
## subset for 120 min timepoint
```{r}
# Subset metadata for time point 120
sample_metadata_120 <- sample_metadata[sample_metadata$time_point == "120", ]

# Check the subsetted metadata
print(sample_metadata_120)

# Subset count matrix for time point 120
count_matrix_120 <- filtered_count_matrix[, colnames(count_matrix) %in% rownames(sample_metadata_120)]

# Check dimensions of the subsetted count matrix
cat("Subsetted count matrix dimensions:", dim(count_matrix_120), "\n")
...
dds <- DESeqDataSetFromMatrix(countData = count_matrix_120,
                              colData = sample_metadata_120,
                              design = ~ condition)
...
write.table(top50_genes, file = "top50_genes_120.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
```
```{r}
...
write.table(bot50_genes, file = "bot50_genes_120.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
...
## Save file as csv
```{r}
write_csv(res_filtered_final, "yeast_results_120.csv")
```
