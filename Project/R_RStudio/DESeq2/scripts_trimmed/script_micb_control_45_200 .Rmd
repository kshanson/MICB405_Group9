...
sample_metadata
```
## subset for time points (45&200) and treatment (control)
```{r}
# Subset metadata for time point 200
sample_metadata_control_45_200 <- sample_metadata[
...
print(sample_metadata_control_45_200)

# Subset count matrix for time point 200
count_matrix_control_45_200 <- filtered_count_matrix[, colnames(count_matrix) %in% rownames(sample_metadata_control_45_200)]

# Check dimensions of the subsetted count matrix
cat("Subsetted count matrix dimensions:", dim(count_matrix_control_45_200), "\n")
...
dds <- DESeqDataSetFromMatrix(countData = count_matrix_control_45_200,
                              colData = sample_metadata_control_45_200,
                              design = ~ time_point)
...
write.table(top50_genes, file = "top50_genes_control_45_200.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
```
```{r}
...
write.table(bot50_genes, file = "bot50_genes_control_45_200.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
...
## Save file as csv
```{r}
write_csv(res_filtered_final, "yeast_results_control_45_200.csv")
```
