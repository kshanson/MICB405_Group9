...
date: "2024-11-19, Updated 2024-11-26"
...
#output_file <- "data/geneID2GO.tsv"
#write.table(gene_to_go, file = output_file, sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)

# Print a message
#message("Gene-to-GO mapping file created successfully!")
...
```{r}
data <- read_csv("data/yeast_results.csv")
```
...
order_term_down <- down_GO_filtered_arranged %>% 
...
down_GO_filtered_arranged %>% 
  ...
  geom_point(size = 3) +
  coord_flip() +
  scale_x_discrete(limits = order_term_down) + 
