...
```{r}
data <- read_csv("data_updated/yeast_results_200.csv")
```
...
```{r}
down_go_200 <- down_GO_filtered_arranged %>% 
  ...
  theme_light() +
   labs(x = "GO Term Description", y = "Enrichment Ratio", color = "P-value", size = "Number of Significant Genes", title = "200 Minutes") +
...
ggsave("down_go_200.png", plot = down_go_200, width = 7, height = 5)
...
up_go_200 <- up_GO_filtered_arranged %>% 
  ggplot(aes(x= Term, y = GeneRatio, colour = weight01)) +
  geom_col(width = 0.05) +
  geom_point(aes(size= Significant)) +
  coord_flip() +
...
ggsave("up_go_200.png", plot = up_go_200, width = 7, height = 5)
