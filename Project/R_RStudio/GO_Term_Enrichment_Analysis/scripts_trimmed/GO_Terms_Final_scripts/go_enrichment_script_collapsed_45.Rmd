...
write.table(geneID2GO, file = "data/collapsed_geneID2GO_45.tsv", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
```{r}
geneID2GO <- readMappings("data/collapsed_geneID2GO_45.tsv")
...
```{r}
data <- read_csv("data_updated/yeast_results_45.csv")
```
...
```{r}
down_go_45 <- down_GO_filtered_arranged %>% 
  ...
  theme_light() +
  labs(x = "GO Term Description", y = "Enrichment Ratio", color = "P-value", size = "Number of Significant Genes", title = "45 Minutes") +
...
ggsave("down_go_45.png", down_go_45, width = 7, height = 5)
...
up_go_45 <- up_GO_filtered_arranged %>% 
  ggplot(aes(x= Term, y = GeneRatio, colour = weight01)) +
  geom_col(width = 0.05) +
  geom_point(aes(size= Significant)) +
  coord_flip() +
...
ggsave("up_go_45.png", up_go_45, width = 7, height = 5)
