...
write.table(geneID2GO, file = "data/collapsed_geneID2GO_trimmed.tsv", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
```{r}
geneID2GO <- readMappings("data/collapsed_geneID2GO_trimmed.tsv")
...
```{r}
data <- read_csv("data/yeast_results_trimmed.csv")
```
...
joined_GO_filtered_arranged %>%
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1) # Tilt numbers with proper alignment
  ) +
  scale_y_continuous(
    breaks = seq(0, 1, 0.2),  # Ensure breaks align with the data
    limits = c(0, 1),         # Ensure the axis covers the range of the data
